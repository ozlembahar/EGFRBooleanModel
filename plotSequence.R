plotSequence = function (network, startState, plot.order = 1:length(startState), includeAttractorStates = c("all", 
    "first", "none"), sequence, title = "", mode = c("table", 
    "graph"), plotFixed = TRUE, grouping = list(), onColor = "#2060b0", 
    offColor = "#e0e0e0", layout, drawLabels = TRUE, drawLegend = TRUE, 
    highlightAttractor = TRUE, reverse = FALSE, borderColor = "black", 
    eps = 0.1, attractor.sep.lwd = 3, attractor.sep.col = "#e06000", 
    ...) 
{
    if (!missing(network)) {
        ...
        sequence <- getPathToAttractor(network = network, state = startState, 
    ...
    switch(match.arg(mode, c("table", "graph")), table = {
        totalMatrix <- t(sequence)[plot.order, ]
        if (length(grouping) > 0) totalMatrix = totalMatrix[unlist(grouping$index), 
            , drop = FALSE]
        if (is.null(colnames(totalMatrix))) {
            colnames(totalMatrix) <- seq_len(ncol(totalMatrix))
        } else if (length(grep("t = ", colnames(totalMatrix)) == 
            ncol(totalMatrix))) {
            colnames(totalMatrix) <- sapply(colnames(totalMatrix), 
                gsub, pattern = "t = ", replacement = "", fixed = TRUE)
        }
        if (!reverse) totalMatrix <- totalMatrix[nrow(totalMatrix):1, 
            , drop = F]
        plot(0, type = "n", xlim = c(0, ncol(totalMatrix)), ylim = c(-2, 
            nrow(totalMatrix) + 1), xlab = "", ylab = "", axes = FALSE, 
            main = title, ...)
        len <- ncol(totalMatrix)
        xStart <- 1
        unitFactor <- (len - (2 * eps))/len
        axis(3, eps, "t = ", lty = "blank", yaxt = "s", xaxt = "s", 
            xaxs = "i", pos = nrow(totalMatrix) - 1)
        axis(3, eps + (1:ncol(totalMatrix)) * unitFactor - unitFactor/2, 
            ((1:ncol(totalMatrix)) - 1), lty = "blank", yaxt = "s", 
            xaxt = "s", xaxs = "i", pos = nrow(totalMatrix) - 1)
        axis(2, seq_len(nrow(totalMatrix)) - 0.5, rownames(totalMatrix), 
            yaxt = "s", xaxt = "s", xaxs = "i", las = 2, cex.axis = 0.8)
        startX <- eps
        for (i in seq_len(ncol(totalMatrix))) {
            for (j in seq_len(nrow(totalMatrix))) {
                rectCol <- ifelse(totalMatrix[j, i], onColor, 
                  offColor)
                rect(startX, j - 1, startX + unitFactor, j, col = rectCol, 
                  border = borderColor, lwd = 2)
            }
        ...
        #         abline(h = sepPos[-length(sepPos)], col = "black", 
        #           lwd = 3)
        #         text(ncol(totalMatrix)/2, sepPos - 0.5, grouping$class, 
        #           cex = 0.9)
        #     }
        # }
        if (!is.null(attributes(sequence)$attractor) && highlightAttractor) {
            attrStart <- min(attributes(sequence)$attractor) - 
                1
            rect(xleft = eps + attrStart * unitFactor, xright = eps + 
                attrStart * unitFactor, ybottom = -0.5, ytop = nrow(totalMatrix) + 
                0.75, lwd = attractor.sep.lwd, border = attractor.sep.col)
            arrows(x0 = eps + attrStart * unitFactor, y0 = nrow(totalMatrix) + 
                0.5, x1 = ncol(totalMatrix) - eps, y1 = nrow(totalMatrix) + 
                0.5, length = 0.1, angle = 20, code = 3, col = attractor.sep.col)
            xpd.prev <- par()$xpd
            par(xpd = T)
            text(eps + attrStart * unitFactor, nrow(totalMatrix) + 
                1, "", pos = 4)
            par(xpd = xpd.prev)
        }
        if (drawLegend) legend(x = 0, y = -2, pch = c(15, 15), 
            col = c(offColor, onColor), legend = c("inactive", 
                "active"), cex = 0.7, horiz = T, xpd = T)
        return(totalMatrix)
    }, graph = {
        ...
        if (!is.null(lastAttractorEdgeIndex)) {
            ...
            graph <- set.edge.attribute(graph, "curved", index = lastAttractorEdgeIndex - 
                bias, value = 0.5)
        }
        plot(graph, layout = layout, vertex.label = labels, vertex.label.cex = args$vertex.label.cex, 
            ...
            vertex.label.dist = args$vertex.label.dist, vertex.label.degree = args$vertex.label.degree, 
